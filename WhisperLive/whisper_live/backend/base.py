...
class ServeClientBase(object):
    ...
    def speech_to_text(self):
        ...
        while True:
            ...
            # Reduced from 1.0s to 0.5s for faster response time
            if duration < 0.5:
            ...
            try:
                ...
                # Don't block on language detection - process audio even if language not yet detected
                if result is None:
                    self.timestamp_offset += duration
                    time.sleep(0.1)    # Reduced sleep time when no voice activity detected
                    continue
                # If language is None but we have a result, language will be auto-detected from the result
                if self.language is None and result:
                    # Language will be set in transcribe_audio via set_language()
                    pass
    ...
    def prepare_segments(self, last_segment=None):
        """
        Prepares ONLY NEW segments that haven't been sent yet.
        
        This prevents cumulative transcription where old segments keep getting re-sent.
        We track how many segments have been sent and only return new ones.
        ...
        Returns:
            list: A list of NEW transcribed text segments to be sent to the client.
        """
        segments = []
        ...
        if total_segments > self.last_sent_segment_count:
            ...
            logging.info(f"📊 Sending {len(segments)} NEW segments (total in transcript: {total_segments})")
        else:
            logging.debug(f"ℹ️ No new completed segments to send (total: {total_segments}, sent: {self.last_sent_segment_count})")
        
        # Add the incomplete last segment (if any)
        if last_segment is not None:
            segments = segments + [last_segment]
            logging.debug(f"📝 Added incomplete last segment: '{last_segment.get('text', '')[:50]}...'")
    ...
    def send_transcription_to_client(self, segments):
        ...
        """
        try:
            # DEBUG: Log transcription before sending
            if segments and len(segments) > 0:
                transcript_text = ' '.join([seg.get('text', '') for seg in segments]).strip()
                logging.info(f"📤 Sending transcription to client {self.client_uid}: {len(segments)} segments, text: '{transcript_text[:100]}...'")
            else:
                logging.warning(f"⚠️ Attempting to send empty segments to client {self.client_uid}")
            ...
            logging.debug(f"✅ Successfully sent {len(segments)} segments to client {self.client_uid}")
        except Exception as e:
            logging.error(f"[ERROR]: Sending data to client {self.client_uid}: {e}")
    ...
    def update_segments(self, segments, duration):
        ...
        logging.info(f"🔍 Processing {len(segments)} segments from Whisper (client: {self.client_uid})")
        for i, s in enumerate(segments):
            no_speech_prob = self.get_segment_no_speech_prob(s)
            logging.info(f"  Segment {i+1}: '{s.text}' (no_speech_prob: {no_speech_prob:.2f}, thresh: {self.no_speech_thresh})")
            if no_speech_prob > self.no_speech_thresh:
                logging.warning(f"  ⚠️ Segment {i+1} filtered out due to high no_speech_prob ({no_speech_prob:.2f} > {self.no_speech_thresh})")
        ...
        if len(segments) > 1 and self.get_segment_no_speech_prob(segments[-1]) <= self.no_speech_thresh:
            logging.info(f"✅ Processing {len(segments)-1} complete segments")
            for s in segments[:-1]:
                text_ = s.text
                ...
                if start >= end:
                    logging.debug(f"⏭️ Skipping segment with invalid time range (start: {start}, end: {end})")
                    continue
                if self.get_segment_no_speech_prob(s) > self.no_speech_thresh:
                    logging.debug(f"⏭️ Skipping segment with high no_speech_prob")
                    continue
                completed_segment = self.format_segment(start, end, text_, completed=True)
                self.transcript.append(completed_segment)
                logging.info(f"✅ Added completed segment to transcript: '{text_}'")
            ...
            if len(segments) <= 1:
                logging.info(f"ℹ️ Only 1 segment, will process as last segment")
            else:
                last_seg_prob = self.get_segment_no_speech_prob(segments[-1])
                logging.warning(f"⚠️ Last segment has high no_speech_prob ({last_seg_prob:.2f}), skipping complete segments")
        ...
        if self.get_segment_no_speech_prob(segments[-1]) <= self.no_speech_thresh:
            ...
            logging.info(f"✅ Last segment added (incomplete): '{self.current_out}'")
        else:
            last_seg_prob = self.get_segment_no_speech_prob(segments[-1])
            logging.warning(f"⚠️ Last segment filtered out due to high no_speech_prob ({last_seg_prob:.2f} > {self.no_speech_thresh})")
