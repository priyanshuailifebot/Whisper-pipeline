...
# merge_segments is not available in faster-whisper 1.2.1, so we implement it
def merge_segments(segments: List[dict], vad_options: Optional[VadOptions] = None) -> List[dict]:
    """
    Merge speech segments that are close together.
    
    Args:
        segments: List of dicts with 'start' and 'end' keys (sample indices)
        vad_options: VAD options (for backward compatibility, not used)
    
    Returns:
        List of merged segments with 'start' and 'end' keys
    """
    if not segments:
        return []
    
    # Sort segments by start time
    sorted_segments = sorted(segments, key=lambda x: x.get('start', x.get('begin', 0)))
    
    # Merge segments that are close together (within 0.5 seconds gap at 16kHz = 8000 samples)
    merged = []
    current_segment = sorted_segments[0].copy()
    
    # Handle both 'start'/'end' and 'begin'/'end' key formats
    start_key = 'start' if 'start' in current_segment else 'begin'
    end_key = 'end'
    
    for next_segment in sorted_segments[1:]:
        gap = next_segment.get(start_key, next_segment.get('begin', 0)) - current_segment.get(end_key, 0)
        
        # Merge if gap is less than 0.5 seconds (8000 samples at 16kHz)
        if gap < 8000:
            # Extend current segment to include next one
            current_segment[end_key] = max(current_segment.get(end_key, 0), 
                                          next_segment.get(end_key, next_segment.get('end', 0)))
        else:
            # Save current segment and start new one
            merged.append({
                'start': current_segment.get(start_key, current_segment.get('begin', 0)),
                'end': current_segment.get(end_key, current_segment.get('end', 0))
            })
        ...
        if not clip_timestamps:
            if vad_filter:
                if vad_parameters is None:
                    ...
                    # Remove invalid parameters (like 'onset' from older versions)
                    valid_vad_params = {
                        'threshold', 'neg_threshold', 'min_speech_duration_ms',
                        'max_speech_duration_s', 'min_silence_duration_ms', 'speech_pad_ms'
                    }
                    # Filter out invalid parameters
                    filtered_params = {k: v for k, v in vad_parameters.items() if k in valid_vad_params}
                    
                    if "max_speech_duration_s" in filtered_params.keys():
                        filtered_params.pop("max_speech_duration_s")

                    vad_parameters = VadOptions(
                        **filtered_params, max_speech_duration_s=chunk_length
                    )
    ...
    def transcribe(
        ...
        if vad_filter and clip_timestamps == "0":
            if vad_parameters is None:
                ...
                # Filter out invalid parameters (like 'onset' from older versions)
                valid_vad_params = {
                    'threshold', 'neg_threshold', 'min_speech_duration_ms',
                    'max_speech_duration_s', 'min_silence_duration_ms', 'speech_pad_ms'
                }
                filtered_params = {k: v for k, v in vad_parameters.items() if k in valid_vad_params}
                vad_parameters = VadOptions(**filtered_params)
